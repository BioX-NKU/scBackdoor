...
def posion_by_trigger(
    adata, target_label, target_label_index, vocab_file="../model/scGPT_human/vocab.json", posion_rate=0.1, topnstop=None
):
    ...
    # match vocab and pick target-posion data
    special_tokens = ["<pad>", "<cls>", "<eoc>"]
    vocab = GeneVocab.from_file(vocab_file)
    for s in special_tokens:
        if s not in vocab:
            vocab.append_token(s)
    p_adata.var["id_in_vocab"] = [
        1 if gene in vocab else -1 for gene in p_adata.var["gene_name"]
    ]
...
def posion_test_data(
    adata, percent=1,vocab_file="../model/scGPT_human/vocab.json",target_label=None, topnstop=None
):
...
def poison_by_gini(adata, poison_rate=0.1, target_label_id=None):
    ...
    list: Indices of cells to be poisoned.
    """
    # Filter genes where 'id_in_vocab' == 1
    gene_mask = adata.var['id_in_vocab'] == 1
    filtered_data = adata[:, gene_mask].X
...
def generate_binary_list(length, indices):
    """
    Generate a list of given length with 1s at specified indices and 0s elsewhere.

    Parameters:
    length (int): The total length of the list.
    indices (list of int): The indices where 1s should be placed.

    Returns:
    list: A list of 0s and 1s with 1s at the specified indices.
    """
